...
test('el usuario ha iniciado sesión', function () {
    $response = $this->getJson("/api/proyectos/100/categorias");
...
test('usuarios sin permiso no estan autorizados', function ($dataset) {
    ...
    $response = $this->actingAs($login)->getJson("/api/proyectos/$proyecto->id/categorias");
    ...
    "Proyecto no vinculado" => function(){
        ...
        /** @var Role $rol */
        $rol = Role::factory()->create();
        $rol->givePermissionTo("Ver categorías");
        $login->assignRole($rol);
...
test('usuarios autorizados', function ($dataset) {
    ...
    "Acceso indirecto" => function(){
        ...
        $permission->givePermissionTo("Ver categorías");
