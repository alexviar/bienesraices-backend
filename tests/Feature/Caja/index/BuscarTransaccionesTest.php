...
test('el usuario ha iniciado sesión', function () {
    $response = $this->getJson('/api/transacciones');
...
test('usuarios sin permiso no estan autorizados', function () {
    ...
    $response = $this->actingAs($login)->getJson('/api/transacciones');
...
test('usuarios autorizados', function ($dataset) {
    ...
    "Acceso directo" => function(){
        ...
        /** @var Role $rol */
        $rol = Role::factory()->create();
        $rol->givePermissionTo("Ver transacciones");
        $login->assignRole($rol);
        return [
            "login" => $login
        ];
    ...
    "Acceso indirecto" => function(){
        ...
        $permission->givePermissionTo("Ver transacciones");
